...
public class JointSerializer extends ReadOnlySerializer<Joint> {
	...
	public Joint read (Json json, JsonValue jsonData, Class type) {
		if (bodies == null || world == null) return null;
		...
		if (indexA >= bodies.size || indexB >= bodies.size) return null;
		...
		if (jointDef != null) {
			...
			jointDef.collideConnected = json.readValue("collideConnected", boolean.class, RubeDefaults.Joint.collideConnected,
				jsonData);
	...
	public class PrismaticJointDefSerializer extends ReadOnlySerializer<PrismaticJointDef> {
		...
		public PrismaticJointDef read (Json json, JsonValue jsonData, Class type) {
			...
			if (anchorA != null && anchorB != null) {
				...
				if (localAxis == null) localAxis = json.readValue("localAxis1", Vector2.class, defaults.localAxisA, jsonData);
				if (localAxis != null) def.localAxisA.set(localAxis);
