...
public class RubeWorldSerializer extends ReadOnlySerializer<RubeScene> {
	...
	public RubeScene read (Json json, JsonValue jsonData, Class type) {
		if (!mScenePopulated) {
			...
			scene.positionIterations = json.readValue("positionIterations", int.class, RubeDefaults.World.positionIterations,
				jsonData);
			scene.velocityIterations = json.readValue("velocityIterations", int.class, RubeDefaults.World.velocityIterations,
				jsonData);
